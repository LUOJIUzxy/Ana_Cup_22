...
rm(list=ls())
getwd()
#setwd('C:\\Users\\Pepijn\\Desktop\\Tum_Classes\\Business Analytics & Machine Learning\\Analytics_cup')

#laoding in all the data
test_data <- read_csv("pub_f6Xd8II.csv")
customers <- read_csv("customers.csv", col_types = cols(CUSTOMER = col_character(),REV_CURRENT_YEAR = col_number()))
geo <- read_csv("geo.csv")
#sumbission_random <- read_csv("submission_random.csv")
transactions <- read_csv("transactions.csv",  col_types = cols(CUSTOMER = col_character()))
...
df_full2 = left_join(df_full, customers, by= c("CUSTOMER", "COUNTRY"))
#view(df_full)
...
# OWNERSHIP
#hash out next one for final submission
...
#SALES_LOCATION
df_full2[c("SALES_LOCATION")][is.na(df_full2[c("SALES_LOCATION")])] <- "Z¨¹rich EPS"
...
avg_all_ISIC <- mean(df_full$ISIC, na.rm = TRUE)
df_full2[c("ISIC")][is.na(df_full2[c("ISIC")])] <- avg_all_ISIC

#df_full2 %>% count(ISIC)
#replacing the missing values with the mean for ISIC
...
task
task.smote = smote(task, rate = 4, nn = 5)
task.smote
...
learner <- makeLearner("classif.randomForest", predict.type = "response", 
                       par.vals = list(ntree = 100L, importance=TRUE))
learner
...
r = resample(learner, task.smote, rdesc, , measures = list(mmce,tpr,fpr,fnr,tnr,acc))
r

#start tuning
#tune <- tuneParams(learner = learner, task = task.smote, resampling = rdesc, 
#                   measures = list(acc), par.set = params, control = ctrl, show.info = T)
#tune$x

learner = setHyperPars(learner, par.vals = list(mtry = 10, nodesize=21))
learner
...
mod = train(learner, task.smote)
mod

preds <- predict(mod, newdata = test)
#preds <- predict(mod, newdata = train_data)
preds
performance(preds, measures = list(mmce,tpr,fpr,fnr,tnr,acc))

predictions = data.frame(id=test_data$id, prediction=preds$data$response)
predictions

######################   Export the Predictions   ################################
write.csv(predictions, file="predictions_brainy_otter_4.csv", row.names=FALSE)
